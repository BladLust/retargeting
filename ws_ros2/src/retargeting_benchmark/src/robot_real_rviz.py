...
class RobotReal:
    def __init__(self, node: Node, virtual_hardware=False, use_high_freq_interp=False):
        ...
        if not virtual_hardware:
            # state subscriber
            # NOTE:disable arm for hand only
            ...
            self.hand_joint_pos_client = self.node.create_client(
                LeapPosition, "/leap_position", callback_group=client_cb_group
            )
    ...
    def _update_hand_joint_pos(self):
        ...
        # --- START OF FIX ---
        # INSTEAD of rclpy.spin_until_future_complete(self.node, future):
        # Passively wait and let the background executor thread do the work.
        # This avoids a conflict between two different threads trying to spin the same node.
        while rclpy.ok() and not future.done():
            time.sleep(0.001)  # A small sleep to prevent pegging the CPU

        if future.done():
            try:
                response = future.result()
                hand_joint_pos = np.asarray(response.position) - np.pi
                self.curr_joint_pos[self.n_arm_joints :] = hand_joint_pos
                print("Heyy it works", hand_joint_pos)
            except Exception as e:
                self.node.get_logger().error(f"Service call to /leap_position failed with exception: {e}")
        # --- END OF FIX ---
    ...
    def update_joint_pos(self):
        ...
        if not self.virtual_hardware:
            ...
            self.curr_joint_pos[: self.n_arm_joints] = self.target_joint_pos[: self.n_arm_joints].copy()
        else:
